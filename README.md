...
- `Close` / `Kill` / `Kill Tree` 前の確認ダイアログと対象PID一覧表示
...
- RAM descending and VRAM descending sort presets
- Search filter
- Quick filters: `All`, `Python`, `GPU Active`, `Local Web`, `Codex/Claude`, `Heavy RAM`, `Heavy VRAM`
- System/protected process hiding
- `Close`, `Kill`, and `Kill Tree` actions
- Row right-click menu for `Open Local Web`, PID/name/path/command-line copy, and termination actions
- Detail panel actions for `Show EXE in Explorer`, `Open EXE Folder`, summary copy, and a lightweight child-process tree
- Confirmation dialog with the target PID list before `Close`, `Kill`, or `Kill Tree`
- Protected process denylist for core Windows processes
- `Settings` window for editing, resetting, opening, and reloading `settings.json`
...
1. Start RunScope.
2. Press `Load / Reload`.
3. Sort by `VRAM High` or `RAM High`.
4. Use quick filters: `All`, `Python`, `GPU Active`, `Local Web`, `Codex/Claude`, `Heavy RAM`, or `Heavy VRAM`.
...
10. Right-click a row to use `Open Local Web`, `Copy PID`, `Copy Process Name`, `Copy Path`, `Copy Command Line`, `Close`, `Kill`, or `Kill Tree`.
11. Use the bottom detail panel for `Show EXE in Explorer`, `Open EXE Folder`, summary copy, and child-process tree inspection.
12. Use `Close`, `Kill`, or `Kill Tree` only after confirming the selected target.
13. Use `Settings` for detailed configuration.
...
The default protected names include:
...
Protected processes are blocked from termination by RunScope.

## Settings

Open `Settings` to edit General, Filters, Protection, Keywords, and About sections. Settings include refresh mode, refresh interval, default sort, table view, Heavy RAM/VRAM thresholds, protected process names, Python detection keywords, and Codex/Claude/Terminal root keywords.

RunScope loads `settings.json` from the executable directory when present. If it is absent, built-in defaults are used. The Settings window can also open `settings.json`, reload it from disk, or reset it to defaults.
